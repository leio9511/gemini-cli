...
2.  **Run and Analyze Failure:** Run the test. It is expected to fail. We must analyze the failure to ensure it's failing for the _expected reason_ (e.g., the state did not transition correctly, the output was wrong). A failure confirms the test is correctly exercising the code.
