...
export class SafePatchTool extends BaseTool<SafePatchToolParams, ToolResult> {
  ...
  constructor(private readonly config: Config) {
    super(
      SafePatchTool.Name,
      'Safe Patch',
      'Applies a set of changes to a file using a unified diff patch. This is the preferred tool for all file modifications.\n\n**Usage Protocol:**\n\n1. To use this tool, you must operate on the latest version of the file available in your context. Identify this by finding the file content with the **highest version number**.\n2. If no versioned content is available, you **MUST** call `read_file` or `read_many_files` first to get it.\n3. When generating the `unified_diff`, you **MUST** include at least 10 lines of unchanged context around each change hunk (equivalent to `diff -U 10`).\n4. **The unchanged context lines in the diff MUST be copied from the source file without any changes. They must be byte-for-byte identical to the corresponding lines in the source file.**\n5. You **MUST** provide the `sha256` hash that was returned with that version as the `base_content_sha256` parameter. This hash acts as a lock; the operation will fail if the file has been modified since you read it.',
      Icon.Pencil,
      {
        properties: {
          file_path: {
